...
def possibleProd():
    ...
    for ntA in range(VarNum):
        for ntB in range(VarNum):
            for ntC in range(VarNum):
                result.append((ntA, ntB, ntC))
...
def calcCYK(w):
    ...
    # For nt -> t
    for i in range(L):
        for nonterm in range(VarNum):
            if existProd(nonterm, w[i], -1):
                X[i][i][nonterm] = True

    validProd = filter(lambda (a, b, c): existProd(a, b, c), possibleProd())
    reversedRange = range(L-1)
    reversedRange.reverse()

    for row in (reversedRange):
        for i in range(row+1):
            j = i + L - row - 1
            for k in (range(L)[i:j]):
                for (A, B, C) in validProd:
                    if X[i][k][B] and X[k+1][j][C]:
                        X[i][j][A] = True
